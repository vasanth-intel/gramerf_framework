...
def check_and_enable_edmm_in_manifest(manifest_file):
    if os.environ["EDMM"] == "1":
        ...
        with open(manifest_file) as f:
            ...
            if not 'use_exinfo' in file_contents:
                add_exinfo = True
        if add_edmm_enable:
        ...
        if add_exinfo:
            exinfo_string = '$ a sgx.use_exinfo = true'
...
def set_vm_max_map_count(vm_max_map_count=65530):
    max_map_count_cmd = f"sudo sysctl -w vm.max_map_count={vm_max_map_count}"
    print(f"\n-- Setting vm.vm_max_map_count value as shown below..\n", max_map_count_cmd)
    exec_shell_cmd(max_map_count_cmd, None)
